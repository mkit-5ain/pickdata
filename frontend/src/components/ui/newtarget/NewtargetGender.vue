...
export default {
	...
	created () {
		this.$eventBus.$on('set_gender_insights', this.setGenderInsights)
	},
	beforeDestroy () {
		this.$eventBus.$off('set_gender_insights', this.setGenderInsights)
	},
	methods: {
		setGenderInsights (data, kpi_base) {
			this.listData.data = []
			data.forEach(item => {
		...
		clickSelect(item) {
			if (item.gender == 'male' || item.gender == 'female'){
				this.$eventBus.$emit('set_new_target_gender', item.gender)
			}
		},
		listSort(key, type) {
