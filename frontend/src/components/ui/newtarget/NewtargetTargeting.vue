			...
			<tbody>
				<tr v-for="(item, index) in listData.data">
					<td class="align_center">
						<button v-on:click="addbtn($el, $event, item)" class="add_btn"></button>
					</td>
					<td>{{ item.object_name }}</td>
					<td>{{ item.object_type_kr }}</td>
...
export default {
	...
	created () {
		this.$eventBus.$on('set_targeting_insights', this.setTargetingInsights)
		this.$eventBus.$on('init_set_targeting', this.initSelectTargeting)
	},
	beforeDestroy () {
		this.$eventBus.$off('set_targeting_insights', this.setTargetingInsights)
		this.$eventBus.$off('init_set_targeting', this.initSelectTargeting)
	},
	methods: {
		initSelectTargeting() {
			this.selectTargeting = []
			this.$eventBus.$emit('set_new_target_targeting', this.selectTargeting)
		},
		addbtn(element, event, item) {
			// console.log('addbtn')
			// console.log(element)
			// console.log(event)
			...
			// console.log(this.selectTargeting.find(item))

			this.selectTargeting = this.pushSelectTargeting(this.selectTargeting, item)
			this.$eventBus.$emit('set_new_target_targeting', this.selectTargeting)
		},
		pushSelectTargeting (selectTargeting, item) {
			var boolPush = true
			for (var i=0; i<selectTargeting.length; i++){
				if (selectTargeting[i] === item) {
					boolPush = false
				}
			}
			if (boolPush) {
				selectTargeting.push(item)
			}

			return selectTargeting
		},
		setTargetingInsights (data, kpi_base) {
			this.listData.data = []
			data.forEach(item => {
...
.table_wrap{height:317px; overflow-y:scroll;
	table{width:100%; border-top:2px solid #000;
		...
		td{height:44px; padding-left:15px; border-right:1px solid #f1f1f1; border-bottom:1px solid #f1f1f1; font-size:12px;
			&:last-child{border-right:0px;}
...
/* 추가 버튼 on off */
.add_btn{background:url("../../../assets/images/icon/newtarget_add_on.jpg") 0 0 no-repeat;
	&.off{background:url("../../../assets/images/icon/newtarget_add_off.jpg") 0 0 no-repeat;}
}
</style>
