<template>
  <div class="target_contents_wrap clearfix" v-if="isShow">
    ...
    <div class="btn_wrap">
      ...
      <button type="button" class="next_btn" @click="createVisitSiteTarget()" v-if="makeType === 'add'">타겟 만들기</button>
      <button type="button" class="delete_btn" @click="deleteVisitSiteTarget()" v-if="makeType === 'modify'">삭제</button>
      <button type="button" class="next_btn" @click="updateVisitSiteTarget()" v-if="makeType === 'modify'">타겟 수정하기</button>
    </div>
  </div>
</template>
